...
export default function Navbar() {
  ...
  const { planIds, savedIds } = useAppContext();
    ...
    <nav className="bg-black border-b border-zinc-800 px-4 sm:px-6">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between">
        <Link href="/" className="flex items-center gap-2 shrink-0">
          <Image src={logo} alt="FitLog logo" width={24} height={24} priority />
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  className={`rounded-full px-4 py-2 text-sm font-medium w-fit transition-colors ${isActive
                      ? "bg-lime-400 text-black"
                      : "text-zinc-400 hover:text-white"
                    }`}
                >
